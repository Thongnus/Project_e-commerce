...
function fetchUsers(page) {
    fetch(`http://localhost:8085/admin/users/listusers?page=${page}`,{
        method: 'GET',
        headers: {
...
function updateTable(userData) {
    ...
    userData.forEach(user => {
      // Lấy mảng các giá trị name từ mỗi đối tượng role
const roleNames = user.roles.map(role => role.name);
var role=null;
if(roleNames=="ROLE_ADMIN"){
    role="ADMIN";
}else{
    role="USER";
}
...
function add() {
    ...
    fetch('http://localhost:8085/admin/users/adduser', {
        method: 'POST',
        headers: {
            'Authorization': 'Bearer ' + token,
            'Content-Type': 'application/json'
...
function deleteRow(id) {
    if (confirm("Bạn có chắc chắn muốn xóa người dùng này không?")) {
        ...
        fetch(`http://localhost:8085/admin/users/delete/${id}`, {
            method: 'DELETE',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
...
function updateUser() {
    ...
    fetch(`http://localhost:8085/admin/users/update/${userId}`, {
        method: 'PUT',
        headers: {
            'Authorization': 'Bearer ' + token,
            'Content-Type': 'application/json'
...
function searchByUsername() {
    ...
    fetch(`http://localhost:8085/admin/users/search?keyword=${searchInput}`,
        {
            method: 'GET',
            headers: {
                'Authorization': 'Bearer ' + token,
